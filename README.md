...
|模块|状态|最新事实|
|---|---|---|
|Stage B续训|RUNNING|已完成step `41000/50000`（82%）；最新验证及当前best均为step41000；3×A100持续运行|
...
一句话概括：**模型训练完成82%，正式数据和公共模型已经备齐，但论文级统一模型对比还没有启动。**

1. **主模型训练**：Stage B已从step14000无替换续训到step41000；step41000的`val_selection_loss=0.9937523`，是当前最佳验证点。相对正式候选起点step16000下降约4.51%，说明预训练目标仍在改善。
...
- [训练状态、27个validation点和解释](TRAINING_PROGRESS.md)
...
最新验证step41000：`val_selection_loss=0.9937523`，是当前最佳验证点。训练仍在继续，不能把step41000称为最终模型，也不能在完整正式评测前声称超过公共强基线。
