...
|模块|状态|最新事实|
|---|---|---|
|Stage B续训|RUNNING|已完成step `44000/50000`（88%）；最新验证及当前best均为step44000；3×A100持续运行|
...
|全量数据审计|RUNNING|SLURM作业`8622446`在q05执行；当前正在扫描C05的8.4 GB样本表，约完成87%；进程仍有真实读写|
|正式协议|DRAFT|固定矩阵3597行；step16000–44000已有15/18个checkpoint身份，等待step46000、48000、50000|
...
它不是模型训练，也不是正式模型对比，而是最后一轮**数据完整性检查**。程序逐项读取53个非EDTA任务的`samples.tsv`，把样本ID、DNA序列指纹和分组写入临时SQLite数据库，检查重复样本、同一序列跨train/validation/test泄漏、候选组跨split泄漏、数据划分是否齐全，并重新核对数据文件哈希。

截至本页时间，作业已运行约20小时2分钟，正在处理七物种染色质开放性任务C05：8.4 GB文件已读到约87.13%。15秒实测仍读取约18.5 MB并写入约8.2 MB，说明进程没有挂死；耗时长的主要原因是大表在共享文件系统上建立SQLite索引。该作业只有2个CPU，正式GPU评测尚未启动。已有18项逐任务检查结果可复用；作业仅余约3小时58分，而C05之后仍有约37.6 GB样本表待审计，因此24小时内全部闭包风险很高。若超时，应只对缺失任务分片补齐，不能从头重复已通过项目。
...
一句话概括：**模型训练完成88%，正式数据和公共模型已经备齐，但论文级统一模型对比还没有启动。**

1. **主模型训练**：Stage B已从step14000无替换续训到step44000；step44000的`val_selection_loss=0.9912032`，是当前最佳验证点。相对正式候选起点step16000下降约4.75%，说明预训练目标仍在改善。
2. **下游数据**：C/D新增39项已经全部转换为统一格式并生成回执；连同既有任务，当前正式非EDTA数据索引为53/53 ready。作业8622446正在做最后一次逐样本泄漏与一致性审计，尚未生成最终审计结论。
...
5. **还差什么**：等待step46000、48000、50000三个正式候选checkpoint、数据审计闭包和最终执行授权。正式比较尚未开始，因此现在不能宣称CropGenome-FM超过公共强模型。
...
完整图覆盖step10–44090，共4409个train点和44个validation点；绿色虚线标出step14000精确续训边界。图中step10–14000来自原Stage B权威源表，step14000之后来自当前无替换续训日志；旧训练中已经被续训替代的step15000–17000没有混入新谱系。原有两张续训图继续保留，作为右半段细节放大图。

最新验证step44000：`val_selection_loss=0.9912032`，是当前最佳验证点。训练仍在继续，不能把step44000称为最终模型，也不能在完整正式评测前声称超过公共强基线。
