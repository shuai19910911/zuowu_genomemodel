...
|模块|状态|最新事实|
|---|---|---|
|Stage B续训|RUNNING|已完成step `43000/50000`（86%）；最新验证及当前best均为step43000；3×A100持续运行|
...
|全量数据审计|RUNNING|SLURM作业`8622446`在q05执行；当前正在扫描C05的8.4 GB样本表，约完成86%；进程仍有真实读写|
...
截至本页时间，作业已运行约18小时51分钟，正在处理七物种染色质开放性任务C05：8.4 GB文件已读到约86%。15秒实测仍读取约19.3 MB并写入约8.6 MB，说明进程没有挂死；耗时长的主要原因是大表在共享文件系统上建立SQLite索引。该作业只有2个CPU，正式GPU评测尚未启动。C05之后仍有约37.6 GB样本表待审计，因此24小时作业时限存在不足风险；后续应复用已完成的逐任务回执，并对未完成任务做分片补齐，不能从头重复已通过项目。
...
一句话概括：**模型训练完成86%，正式数据和公共模型已经备齐，但论文级统一模型对比还没有启动。**

1. **主模型训练**：Stage B已从step14000无替换续训到step43000；step43000的`val_selection_loss=0.9919196`，是当前最佳验证点。相对正式候选起点step16000下降约4.68%，说明预训练目标仍在改善。
