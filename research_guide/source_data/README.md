...
| `stage_b_shard_sampling_audit.tsv` | 42个Stage B分片逐项记录碱基数、总/训练/验证/测试片段数、4K/8K/16K组成、7类区域的全部/训练片段数及旧读取器下单条训练片段的相对抽样权重 |
| `stage_b_region_training_coverage.tsv` | 7类区域的全部片段、训练片段、训练池占比、旧读取器单次抽样概率，以及到step14000/17000的预计抽取次数；明确标记实际逐区域抽取计数未被历史日志记录 |
| `stage_b_split_assembly_coverage.tsv` | 质量筛选后真正贡献Stage B片段的训练/验证/测试候选基因组版本数和片段数 |
...
`stage_b_shard_sampling_audit.tsv`、`stage_b_region_training_coverage.tsv`和`stage_b_split_assembly_coverage.tsv`均由`../scripts/build_stage_b_shard_sampling_audit.py`逐个读取本地冻结Stage B窗口索引生成。脚本验证42个压缩索引的SHA-256、manifest片段数和按4K/8K/16K重算的碱基总数，再按真实旧读取器公式计算抽样权重、7类区域预计抽取次数和实际贡献片段的基因组版本数。预计次数是从已验证读取器推导的数学期望，不是历史训练日志中的实际计数。公开表不包含DNA序列；原始约41.24 GB碱基编码和窗口索引仍不提交到GitHub。
