...
2026-08-08 20:14 CST快照：106/1572行、9/251个GPU组闭合，终止失败0。CPU-only下游任务在gpu05本机CPU执行：调度daemon运行中，1个worker活跃、51行就绪，全部隐藏CUDA设备、按内存限流；SLURM q05提交已禁用且q05无本项目作业。gpu05保留1个既有GPU组继续运行，新GPU派发保持暂停。

最终审计当前为`BLOCKED`：实现哈希漂移、当前协议绑定的正式smoke回执缺失和矩阵未闭合仍待处理；此前14/14公共模型真实GPU前向冒烟已经通过，但不能替代正式回执。gpu05的B4:00.0无法完成驱动初始化，NVML只看到其余7/8块卡；一次在空闲健康卡上的受控测试也卡在新CUDA上下文初始化，而既有B5:00.0任务仍可推进。GPU任务排空后由管理员完整重启/冷启动并逐卡验证，当前证据不能直接断言永久物理坏卡。
