# formal_caduceus_rc_stageB_step5000 downstream first-pass probe

更新时间: 2026-06-22 10:45 CST

## 1. 版本边界

本页记录正式 CaduceusRC（反向互补一致性）Stage_B（第二阶段预训练）step5000 checkpoint（模型存档点）的同口径 128 bp（128 个碱基对）first-pass downstream probe（第一轮下游探针评测）。

| 项目 | 内容 |
|---|---|
| Version ID | `formal_caduceus_rc_stageB_step5000` |
| checkpoint（模型存档点） | `training_server_transfer/runs/Stage_B_formal_caduceus_rc/checkpoints/step_00005000.pt` |
...
1. `CropGenome-FM formal CaduceusRC frozen embedding`（正式 CaduceusRC 冻结表示）：加载 step5000 checkpoint（模型存档点），不微调 backbone（主干模型），抽取 mean-pooled embedding（平均池化向量表示），用 nearest-centroid（最近质心）分类。
...
|---|---:|---:|---:|---:|---:|
| CropGenome-FM formal CaduceusRC frozen embedding | 0.267857 | 0.242750 | 0.267857 | 56 | 56 |
| 1-mer composition baseline | 0.178571 | 0.147429 | 0.178571 | 56 | 56 |
| Delta（模型 - 基线） | +0.089286 | +0.095321 | +0.089286 | - | - |

结论：step5000 的 frozen embedding（冻结表示）在 128 bp（碱基对）probe（探针评测）中高于 1-mer baseline（单碱基组成基线）。这支持当前 checkpoint（模型存档点）仍有区域表示信号，但该结果仍是小样本 first-pass probe（第一轮探针评测），不能当作正式 benchmark（基准评测）结论。

## 4. 分类别 F1

| Class（类别） | Support（测试样本数） | Precision（精确率） | Recall（召回率） | F1 |
|---|---:|---:|---:|---:|
| coding（编码区） | 8 | 0.142857 | 0.125000 | 0.133333 |
| splice（剪接区域） | 8 | 0.000000 | 0.000000 | 0.000000 |
| promoter（启动子） | 8 | 0.285714 | 0.250000 | 0.266667 |
| utr（非翻译区） | 8 | 0.090909 | 0.125000 | 0.105263 |
| tes（转录终止区域） | 8 | 0.153846 | 0.250000 | 0.190476 |
| gene_body（基因主体） | 8 | 0.285714 | 0.250000 | 0.266667 |
| background（背景区域） | 8 | 0.636364 | 0.875000 | 0.736842 |
