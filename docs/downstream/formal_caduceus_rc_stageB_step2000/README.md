# formal_caduceus_rc_stageB_step2000 downstream first-pass probe

更新时间: 2026-06-19 21:29 CST

## 1. 版本边界

本页记录正式 CaduceusRC（反向互补一致性）Stage_B（第二阶段预训练）step2000 checkpoint（模型存档点）的同口径 128 bp（128 个碱基对）first-pass downstream probe（第一轮下游探针评测）。

| 项目 | 内容 |
|---|---|
| Version ID | `formal_caduceus_rc_stageB_step2000` |
| checkpoint（模型存档点） | `training_server_transfer/runs/Stage_B_formal_caduceus_rc/checkpoints/step_00002000.pt` |
| checkpoint step（训练步） | 2000 |
...
1. `CropGenome-FM formal CaduceusRC frozen embedding`（正式 CaduceusRC 冻结表示）：加载 step2000 checkpoint（模型存档点），不微调 backbone（主干模型），抽取 mean-pooled embedding（平均池化向量表示），用 nearest-centroid（最近质心）分类。
...
|---|---:|---:|---:|---:|---:|
| CropGenome-FM formal CaduceusRC frozen embedding | 0.125000 | 0.088710 | 0.125000 | 56 | 56 |
| 1-mer composition baseline | 0.178571 | 0.147429 | 0.178571 | 56 | 56 |
| Delta（模型 - 基线） | -0.053571 | -0.058719 | -0.053571 | - | - |

结论：step2000 的 frozen embedding（冻结表示）在 128 bp（碱基对）probe（探针评测）中低于 1-mer baseline（单碱基组成基线），提示该小样本 probe 有明显波动，不能解释为能力稳定提升。
...
| promoter（启动子） | 8 | 0.166667 | 0.125000 | 0.142857 |
| utr（非翻译区） | 8 | 0.000000 | 0.000000 | 0.000000 |
| tes（转录终止区域） | 8 | 0.000000 | 0.000000 | 0.000000 |
| gene_body（基因主体） | 8 | 0.210526 | 0.500000 | 0.296296 |
| background（背景区域） | 8 | 0.142857 | 0.250000 | 0.181818 |
