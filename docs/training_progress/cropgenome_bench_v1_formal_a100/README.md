...
6. **Stage C1 64K 工程 gate（进入条件）通过。**A100 GPU2 已真实完成 64K 单 batch 的前向、反向和 optimizer step（参数更新），峰值显存分配 `29,240.5 MiB`、预留 `30,762.0 MiB`，没有 OOM（显存溢出）。
...
1. 论文结果保留两个 checkpoint，不宣称“全任务唯一最佳”。
2. Stage C1 初始化继续使用预先由 validation selection loss（验证选择损失）选出的 `checkpoint_best.pt = step14000`，不能因为看过 formal test 后再用 test 反向挑初始化点。
3. step17000 保留为 sensitivity/backup（敏感性/备选），用于检验结论是否依赖单一 checkpoint。
...
| 总 loss | `0.631388` |
| MLM loss | `0.487883` |
| 峰值 allocated 显存 | `29,240.5 MiB` |
| 峰值 reserved 显存 | `30,762.0 MiB` |
...
- 是否已经证明 64K 长上下文有效：**没有**。dry-run 只证明“装得下、能更新”，不证明 64K 比 8K 更准。
- 是否可以进入 Stage C1 正式训练：**GO**。初始化应使用 validation-best step14000，并在 Stage C1 设置独立验证和停止规则。
