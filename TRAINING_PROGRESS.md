...
|最新公开里程碑|step41000 / 50000（82%）|
|最新验证点|step41000|
|当前最佳验证点|step41000|
|best val selection loss|0.9937523|
...
|正式候选身份|13/18已哈希登记：step16000–40000；等待step42000–50000|
...
从正式候选起点step16000到step41000，validation selection loss由1.0406386降至0.9937523，下降0.0468863（约4.51%）。总体趋势继续改善，但total loss和辅助region指标并非每个点都严格单调，最终模型价值仍必须由固定下游任务验证。
...
|正式运行计划|固定矩阵3597行、5个seed；checkpoint身份13/18；完整正式评测尚未启动|
...
5. step42000–50000到达后补齐剩余5个checkpoint身份，再在同数据、同split、同上下文和同下游头预算下统一运行公共模型与候选checkpoint；不能用反复查看test的方式悄悄挑最优模型。
