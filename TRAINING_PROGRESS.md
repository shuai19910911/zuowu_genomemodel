...
|Stage C1最新训练日志|step17500 / 30000（58.33%）|
|Stage C1最新validation|step17500；val selection loss 0.8875320，当前Stage C1最低|
|Stage C1上下文|4K/8K/16K/32K/64K按预设比例混合的单一连续run|
|Stage C1曲线范围|step10–17500；1750个train点、35个validation点|
...
|三checkpoint下游|RUNNING / AUDIT + HARDWARE BLOCKED：54项可执行非EDTA任务、1572行、251个GPU组；快照时106行和9组闭合，终止失败0|
...
从step500到step17500，validation selection loss下降0.0189308（约2.09%），total loss下降约3.97%。35个validation点总体改善，step17500为当前最低；但预训练loss仍不能替代正式下游证据。训练raw loss会受4K–64K混合长度与每步样本组成影响，判断趋势应优先看滚动中位数和固定validation。
...
|三checkpoint正式运行|step40000/45000/50000；1572行、251个GPU组、5个seed；2026-08-08 20:14 CST快照为106行和9组闭合，终止失败0|
...
|GPU调度|gpu05保留1个既有run-group继续运行；新GPU派发daemon保持暂停；受控健康卡测试已确认新CUDA上下文仍会卡死，不再重复探针|

最终审计当前为`BLOCKED`：实现哈希漂移、当前协议绑定的正式smoke回执缺失和1572行矩阵未闭合仍待处理；这不等于此前14/14公共模型真实GPU前向冒烟没有完成。gpu05有8块物理2080 Ti，但B4:00.0（device minor 6）无法完成驱动初始化、VBIOS不可读，NVML只列出其余7块；一次在空闲健康卡上的受控测试也卡在新CUDA上下文初始化，而既有B5:00.0 worker仍在推进。现有worker完成后需要管理员完整重启/冷启动并逐卡验证；只有重启后仍失败才进一步判断显卡、供电或PCIe硬件，当前不能直接称为永久物理坏卡。
...
EDTA继续在q04执行，RepeatModeler明确禁用；119个目标中已有78个满足完整输出与回执条件，8个正在运行、33个排队，最终汇总manifest尚未形成。B11/B12只在最终TE manifest、ID回映射和坐标质控全部通过后启动，不使用中间文件或替代标签。

## 7. 下一步

1. Stage C1继续按4K/8K/16K/32K/64K混合长度单一连续run训练，并按冻结门禁监控validation。
2. 保护gpu05既有GPU与CPU worker完成；gpu05本机CPU队列继续消化CPU-only行。GPU任务排空后由管理员完整重启/冷启动并逐卡验证CUDA，再恢复剩余GPU组、B17敏感性分析和最终审计。
