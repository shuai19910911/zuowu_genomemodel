...
|许可证Gate|23项来源需要在论文中披露；执行授权不改变上游许可证本身|
|模型/基线|18个，包括CropGenome-FM、植物模型、通用DNA模型和k-mer基线|
|正式运行计划|当前草案3831行；完整正式评测尚未启动|
...
4. 完成no-region对照训练并冻结全部模型身份。
5. 在同数据、同split、同上下文和同下游头预算下统一运行公共模型与候选checkpoint；不能用反复查看test的方式悄悄挑最优模型。
