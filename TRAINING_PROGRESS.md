...
|最新训练点|step38000 / 50000|
|最新验证点|step38000|
|当前最佳验证点|step38000|
|best val selection loss|0.9973677|
...
从step15000到38000，validation selection loss由1.0402384降至0.9973677，整体趋势继续改善，但中间并不是每个点都严格单调下降。
...
|EDTA阻塞|B11、B12|
|新增公开任务|C/D共39项；C01–C31数据已整理，C32–C36与D01–D03共8项等待SLURM|
|原始资产|11/11数据源已下载校验；14/14公共模型已下载并通过真实GPU前向冒烟|
|许可证Gate|剩余11项数据任务来自3个未声明许可证的数据卡，另有2个公共模型卡未声明；执行授权不改变上游许可证本身|
|模型/基线|16个：CropGenome-FM、14个公共模型和k-mer简单基线；不含内部消融|
|正式运行计划|当前草案3597行；完整正式评测尚未启动|
...
- C01–C31已经转换为统一数据格式并生成回执；
- C32–C36和D01–D03已提交到q02–q05，当前因资源优先级排队。
...
2. 等待q02–q05完成剩余8项C/D数据整理，然后运行39项统一数据审计。
3. 持续完成EDTA；最终manifest关闭后运行B11/B12。
4. 不运行no-region、random-init或其他内部模型消融，把计算资源集中到作物专属下游任务和公共强模型公平比较。
5. 冻结主模型checkpoint身份后，在同数据、同split、同上下文和同下游头预算下统一运行公共模型与候选checkpoint；不能用反复查看test的方式悄悄挑最优模型。
